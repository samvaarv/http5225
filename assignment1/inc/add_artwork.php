<?php
    ...
    try {
        ...
        $location_stmt = $connect->prepare("INSERT INTO Locations (`Location`, Ward, WardFullName) VALUES (?, ?, ?)");
        $location_stmt->bind_param("sis", $Location, $Ward, $WardFullName);
        ...
        $artwork_stmt = $connect->prepare("INSERT INTO Artworks (Title, Medium, ArtForm, `Status`, ImageName, ImageURL, YearInstalled, `Description`, ImageOrientation)
                                        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)");
        $artwork_stmt->bind_param("ssssssiss", $Title, $Medium, $ArtForm, $Status, $ImageName, $ImageURL, $YearInstalled, $Description, $ImageOrientation);
